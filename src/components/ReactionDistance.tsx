...
export function ReactionDistance({ distanceMetres, carLengthMetres, moving }: ReactionDistanceProps) {
  ...
  const carCountLabel = totalCars > 10 ? "10+" : totalCars.toString();
  ...
  return (
    <section className="reaction-panel" aria-label="Reaction distance">
      ...
      <div className="car-distance-strip">
        ...
        <div className="car-count" aria-label={`${totalCars} car lengths`}>
          <strong>{carCountLabel}</strong>
          <span>car lengths</span>
        </div>
      </div>
    </section>
  );
